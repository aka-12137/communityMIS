...
<div class="container clearfix">
    ...
    <div class="main-wrap">
        ...
        <div class="result-wrap" style="background:#efefef;">
            <form name="myform" id="myform" method="post">
                <div class="result-title">
                    <div class="result-list">
                    
                        <font style="background: none;border: 0;color: red;font-weight: 900;font-size: 24px;float:right;"><s:actionmessage /></font>
                        <form action="user_user_list.action" method="post">
                   						   用户名：  <input class="common-text required" id="title" 
                        ...
                        <tr>
                           ...
                           <c:forEach items="${userlist }" var="list">
							<tr>
								...
								<td style="text-align: center;">
									<c:if test="${list.mean==1 }">非用户</c:if>
									<c:if test="${list.mean==2 }">普通用户</c:if>
								</td>
								<td style="text-align: center;">
									<c:if test="${list.mean==1 }">
									<a href="user_user_audit.action?user.id=${list.id}"
										class="tablelink">审核成为用户</a>&nbsp;&nbsp;
									</c:if>
									<a href="user_user_edit.action?user.id=${list.id}"
										class="tablelink">修改</a>&nbsp;&nbsp;
									<a href="user_user_del.action?user.id=${list.id}"
					...
					<ul class="paginList">
						...
						<c:choose>
							<c:when test="${currentPage == 1 }">
								<li class="paginItem">
									<a
										href="user_user_list.action?currentPage=1"
							...
							<c:otherwise>
								<li class="paginItem">
									<a
										href="user_user_list.action?currentPage=${currentPage-1 }"
										style="width: 80px;">上一页</a>
								</li>
							</c:otherwise>
						</c:choose>
						...
						<c:choose>
							<c:when test="${currentPage == totalPage }">
								<li class="paginItem">
									<a
										href="user_user_list.action?currentPage=${totalPage}"
